...
function can_move(chessboard, i, j, col){
	if(chessboard[i][j]==0){return false;}
	if(chessboard[i][j].get_color()!=col){return false;}
	mow = chessboard[i][j].moves();
	if(mow.length==0){return false;}
	return true;
}
...
//main game loop
	if(selected){
	//moving the piece
		if(white_move==0){move_to = get_square(options.e.clientX, options.e.clientY);}
	...
	if(white_move==1){
		...
		legal_moves=chessboard[moved_from[0]][moved_from[1]].moves();
		do{
			move_to = [Math.floor(8*Math.random()), Math.floor(8*Math.random())];
		}while(!my_includes(legal_moves, move_to) || EOG!="");
	}
});